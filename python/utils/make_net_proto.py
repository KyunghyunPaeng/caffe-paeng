...
def make_vggm_prototxt_for_attention_net(file_name, num_classes, batch_size, phase) :
	net = caffe.NetSpec()
	net.data, net.cls_label = L.AttentionData(root_folder="/data/PASCAL/VOCdevkit/VOC2007/", source="1__DATA/PASCAL/train.txt", 
							  batch_size=batch_size, num_class=num_classes, input_size=224, cache_images=0,
							  mean_value=[104,117,123], ntop=2)
	...
	# net start !!
	net.conv1, net.relu1 = conv_relu(net.data, 7, 96, stride=2)
	net.norm1 = L.LRN(net.conv1, local_size=5, alpha=0.0005, beta=0.75, k=2)
	net.pool1 = max_pool(net.norm1, 3, stride=2)
	
	net.conv2, net.relu2 = conv_relu(net.pool1, 5, 256, pad=1, stride=2)
	net.norm2 = L.LRN(net.conv2, local_size=5, alpha=0.0005, beta=0.75, k=2)
	net.pool2 = max_pool(net.norm2, 3, stride=2)
	
	net.conv3, net.relu3 = conv_relu(net.pool2, 3, 512, pad=1)
	net.conv4, net.relu4 = conv_relu(net.conv3, 3, 512, pad=1)
	net.conv5, net.relu5 = conv_relu(net.conv4, 3, 512, pad=1)
...
def make_vgg_prototxt_for_attention_net(file_name, num_classes, batch_size, phase) :
	net = caffe.NetSpec()
	net.data, net.cls_label = L.AttentionData(root_folder="/data/PASCAL/VOCdevkit/VOC2007/", source="1__DATA/PASCAL/train.txt", 
...
def make_attention_prototxt(proto_file_name, model, phase='TRAIN') :
	...
	if model is 'vgg16':
		make_vgg_prototxt_for_attention_net(file_name, num_classes, batch_size, phase)
	elif model is 'vggm' :
...
if __name__ == '__main__' :
	...
	#make_attention_prototxt(prototxt_file_name, 'vgg16', 'TEST')
