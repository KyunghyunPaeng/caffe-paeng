...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  this->param_propagate_down_[2] = false; // no backprop. mean
  this->param_propagate_down_[3] = false; // no backprop. variance
}
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // put the squares of bottom into buffer_blob_ (X^2)
  caffe_powx(bottom[0]->count(), const_bottom_data, Dtype(2), buffer_blob_.mutable_cpu_data());

  // mean subtraction step
  // mean across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, height_ * width_, Dtype(1. / (height_ * width_)), 
	  const_bottom_data, spatial_sum_multiplier_.cpu_data(), Dtype(0), spatial_statistic_.mutable_cpu_data());
  // mean across batch
